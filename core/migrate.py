...
#: The list of migrations this code expects to ship with, and their bytes.
#:
#: ``pending()`` is ledger-versus-disk, so it treats the directory as the truth of
#: the expected history. `cli/main.py` already refuses an *absent*
#: ``supabase/migrations`` for that reason — "a deployment built without it looks
#: healthy, because nothing is pending when nothing is visible". The partial case
#: is the same fail-open through a narrower door and was not covered: a blank
#: database plus an image missing one file applies the rest, reports
#: ``schema_current: true``, and the objects that file creates — RLS policies, the
#: approvals table that `CLAUDE.md` §4.1 rests on — simply are not there.
#:
#: The manifest is the expected history, checked in next to the files it names, and
#: regenerated by `make migrations-manifest`. It closes the dangerous direction only:
#: a file the manifest names must be present and unmodified. A file on disk that the
#: manifest does not name is not a fail-open (the image ships *more* than expected),
#: so it is a CI gate rather than a boot refusal — otherwise authoring a migration
#: would break the author's own stack before they could regenerate anything.
MANIFEST_NAME = "MANIFEST.sha256"
MANIFEST = MIGRATIONS_DIR / MANIFEST_NAME
...
class IncompleteMigrations(MigrationError):
    """The shipped migrations directory is missing files the manifest names."""

    def __init__(self, missing: Sequence[str], drifted: Sequence[str]) -> None:
        self.missing = list(missing)
        self.drifted = list(drifted)
        parts = []
        if self.missing:
            parts.append(f"absent: {', '.join(self.missing)}")
        if self.drifted:
            parts.append(f"modified: {', '.join(self.drifted)}")
        super().__init__(
            "refusing to migrate: the shipped migrations do not match the manifest, "
            "so an incomplete history would be applied and then reported as current "
            f"({'; '.join(parts)})"
        )
...
def read_manifest(path: Path = MANIFEST) -> dict[str, str]:
    """The expected history: ``{filename: sha256}``, in the format ``sha256sum`` writes.

    An absent manifest yields ``{}``, which verifies vacuously — the same posture the
    ledger takes on a database that has none. The gate against an absent manifest is
    the CI freshness test, not a boot refusal: a runtime that refuses to start on a
    file it cannot find is how an operator loses a stack over a packaging detail.
    """
    if not path.is_file():
        return {}
...
def render_manifest(directory: Path = MIGRATIONS_DIR) -> str:
    """The manifest text for what is on disk now — the generator, in one place."""
    return "".join(f"{checksum(p)}  {p.name}\n" for p in _migration_files(directory))


def verify_manifest(directory: Path = MIGRATIONS_DIR) -> tuple[list[str], list[str]]:
    """``(missing, drifted)`` — files the manifest names that this image does not carry.

    The manifest is read from ``directory`` itself: it ships next to the files it
    names, so a directory is always checked against its own expected history rather
    than against the repository's. Only the dangerous direction is reported — a file
    present on disk and absent from the manifest is not (see :data:`MANIFEST`).
    """
    expected = read_manifest(directory / MANIFEST_NAME)
    missing = [name for name in expected if not (directory / name).is_file()]
    drifted = [
        name
        for name, digest in expected.items()
        if (directory / name).is_file() and checksum(directory / name) != digest
    ]
    return sorted(missing), sorted(drifted)


def _require_complete_migrations(directory: Path) -> None:
    """Refuse to run against a truncated migrations directory (fail-closed)."""
    missing, drifted = verify_manifest(directory)
    if missing or drifted:
        raise IncompleteMigrations(missing, drifted)
...
def apply_all(
    ...
    _require_complete_migrations(directory)
...
def _object_exists(conn: psycopg.Connection, sentinel: Sentinel) -> bool:
    ...
    if sentinel.kind == "policy":
        ...
        # `public` comme les deux autres sondes. Sans ce filtre, un `audit_log`
        # portant le même trigger dans N'IMPORTE QUEL schéma — une archive, une
        # restauration partielle, un `staging` — satisfait le sentinelle : mesuré,
        # cinq fichiers adoptés alors que `public.audit_log` n'existe pas, donc la
        # table d'audit append-only et ses triggers anti-mutation (`CLAUDE.md` §4.2)
        # déclarés appliqués sans l'être.
        query = (
...
def adopt_baseline(conn: psycopg.Connection, directory: Path = MIGRATIONS_DIR) -> list[str]:
    ...
    *contiguous* prefix whose sentinels are all present — those files are not
    ...
    with _runner_lock(conn):
        _ensure_ledger(conn)
        # Un fichier baseline absent du disque était silencieusement retiré du
        # contrôle de contiguïté : l'état mixte cessait alors d'être détectable.
        # On ne peut pas adopter une histoire qu'on est incapable de checksummer.
        absent = [s.filename for s in _BASELINE if not (directory / s.filename).exists()]
        if absent:
            raise MigrationError(
                "refusing to adopt a baseline: these pre-ledger migrations are not on "
                f"disk, so their history cannot be verified: {', '.join(absent)}"
            )
        probed = [(s, _object_exists(conn, s)) for s in _BASELINE]
