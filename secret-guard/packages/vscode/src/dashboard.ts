...
const REASONS: Record<HookHealth["reason"], string> = {
...
<section aria-labelledby="actions"><div class="section-heading"><h2 id="actions">Un doute avant de partager ?</h2><span>Analyse à votre demande</span></div><div class="actions"><a class="action" href="command:secretGuard.scanClipboard"><span class="action-icon" aria-hidden="true">📋</span><span><strong>Vérifier le presse-papiers</strong><small>Scannez le texte que vous allez coller.</small></span></a><a class="action" href="command:secretGuard.scanDocument"><span class="action-icon" aria-hidden="true">🔎</span><span><strong>Vérifier le document</strong><small>Analysez le fichier ouvert dans l’éditeur.</small></span></a></div></section>
<section aria-labelledby="mode"><div class="section-heading"><h2 id="mode">Votre mode de protection</h2><a class="button" href="command:secretGuard.chooseMode">Changer de mode ▾</a></div><div class="policy"><strong>${warnMode === "allow" ? "⚠️ Mode permissif (ancien réglage)" : modeLabel(warnMode)}</strong><p>${warnMode === "allow" ? "Les détections ambiguës peuvent être transmises. Les secrets classés BLOCK restent bloqués." : PROTECTION_MODES.find((entry) => entry.mode === warnMode)?.description}</p><p>${warnMode === "allow" ? "Choisissez un mode pour remplacer cet ancien réglage." : PROTECTION_MODES.find((entry) => entry.mode === warnMode)?.detail}</p></div><p class="note">Réglage commun aux assistants de cette installation. Après un changement, ouvrez une nouvelle session de votre assistant ; Codex peut demander de valider le hook actualisé.</p></section>
